...
class TranslationDriver(SimpleTaskEngine):
    ...
    """  def possibly_check_for_omr(self):
        if self.config.translation.gc == "omr":
            from rpython.rtyper.tool.rffi_platform import configure_omr
            from rpython.translator.platform import CompilationError
            try:
                configure_omr(self.translator.platform)
            except CompilationError as e:
                i = 'OMR GC not installed.  Try e.g. "translate.py --gc=minimark"'
