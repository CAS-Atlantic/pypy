...
class OMRGcPolicy(BasicGcPolicy):

    def gettransformer(self, translator, gchooks):
        from rpython.memory.gctransform import omr
        return omr.OMRGCTransformer(translator)
    ...
    def rtti_type(self):
        return OMRGcRuntimeTypeInfo_OpaqueNode.typename

    def rtti_node_factory(self):
        return OMRGcRuntimeTypeInfo_OpaqueNode

    def compilation_info(self):
        eci = BasicGcPolicy.compilation_info(self)

        from rpython.rtyper.tool.rffi_platform import configure_omr
        eci = eci.merge(configure_omr())

        return eci

    def gc_startup_code(self):
        return omr_gc_Startup()
    ...
    def OP_GC__COLLECT(self, funcgen, op):
        return free_object_memory(), free_root_memory()
...
class OMRGcRuntimeTypeInfo_OpaqueNode(ContainerNode):
    nodekind = 'omr rtti'
    globalcontainer = True
    typename = 'char @'
    _funccodegen_owner = None
