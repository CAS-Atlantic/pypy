...
def configure_omr(platform=None):
    if platform is None:
        from rpython.translator.platform import platform
    if sys.platform == 'win32': # Forget my support for windows fellas
        platform = None
    else:
        library_dir = ''
        libraries = ['omr/pypy']
        includes=['omr/pypy/omrgcpypy.h']
    eci = ExternalCompilationInfo(
        platform=platform,
        includes=includes,
        libraries=libraries,
    ...
    return configure_external_library(
        'omr/pypy', eci,
        [dict(prefix='omrgc-', include_dir='include', library_dir=library_dir)],
        symbol='GC_init')
